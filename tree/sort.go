...
func swap(x []float32, inx []int, i, j int) {
	x[i], x[j] = x[j], x[i]
	inx[i], inx[j] = inx[j], inx[i]
}

// Insertion sort
func insertionSort(x []float32, inx []int, a, b int) {
	for i := a + 1; i < b; i++ {
...
func siftDown(x []float32, inx []int, lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
...
func heapSort(x []float32, inx []int, a, b int) {
	first := a
	lo := 0
	hi := b - a
...
func medianOfThree(x []float32, inx []int, a, b, c int) {
	m0 := b
	m1 := a
	m2 := c
...
func swapRange(x []float32, inx []int, a, b, n int) {
	for i := 0; i < n; i++ {
		swap(x, inx, a+i, b+i)
	}
}

func doPivot(x []float32, inx []int, lo, hi int) (midlo, midhi int) {
...
func quickSort(x []float32, inx []int, a, b, maxDepth int) {
	for b-a > 7 {
		if maxDepth == 0 {
			heapSort(x, inx, a, b)
			return
